...
from ultralytics.utils.metrics import OKS_SIGMA, RLE_WEIGHT, bbox_nwd
...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        stride_fg = stride.squeeze(-1).unsqueeze(0).expand_as(fg_mask)[fg_mask].unsqueeze(-1)
        nwd = bbox_nwd(pred_bboxes[fg_mask] * stride_fg, target_bboxes[fg_mask] * stride_fg)
        loss_iou = ((1.0 - nwd) * weight).sum() / target_scores_sum
